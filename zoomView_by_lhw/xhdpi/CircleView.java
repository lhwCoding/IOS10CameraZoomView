...
public class CircleView extends View {
    ...
    private final float arcLength = 60;//90; // 120  draw center circle length
    private final float totalRotate = 60;//90; // 60 The rest of the rotation Angle
   ...
   private long endTime=0;
    private void init() {
        ...
        canvas = new Canvas();
        currentAngle = 0f;
        startAngle =30f;// 0f;//30f; // init start angle 
    ...
    protected void onDraw(Canvas canvas) {
        ...
        cy = getBottom();
        // Circle radius
        radius = (getRight() - getLeft() - getPaddingRight() - getPaddingLeft())/2+20;
        cx = getWidth()/2;
        paint.setColor(Color.GRAY);
        paint.setAlpha(90);
      //  Log.i("LHW", "cy"+cy+ ", cx=" +cx);// 1040 360
        // draw gray backgroud color
        this.canvas.drawCircle(cx, cy+200, radius, paint);
   /*     RectF oval=new RectF();  
       ...
       // Log.i("CircleView", "getRight"+getRight() + ", getBottom=" + getBottom());
        ...
        downX=cx;
        downY=cy - radius;
        this.canvas.drawCircle(cx, cy - radius+200, centerRaidus-8, paint);
...
//        Region region = new Region((int) (cx - radius), (int) (cy - radius), (int) (cx + radius), (int) (cy + radius));
        ...
        this.canvas.drawText( showZoomValue+"x", cx -18 , cy - radius+200, paint);
	...
	private void drawCircle() {
		for (float a = startAngle; a < endAngle; a += 2) {
            ...
            if (!rect.contains((int)smallCx, (int)smallCy)){
            	 ...
            	 if(a==startAngle+2){
            		 this.canvas.drawText("10x",smallCx, smallCy+200, paint); 
            	 }else if(a==endAngle-2){
            		 this.canvas.drawText("1x",smallCx-40, smallCy+200, paint); 	 
            	 }else{
            		 this.canvas.drawCircle(smallCx, smallCy+200, 4, paint);
            	 }
    ...
    private void setAngle(float scaleAngle) {
     ...
     //  Log.i("CircleView", "startAngle="+startAngle +" rotatAngle="+rotatAngle);
       ...
       // Log.i("CircleView", "startAngleTemp="+startAngleTemp +" endAngleTemp="+endAngleTemp);
        ...
        if (startAngleTemp >= 30f && startAngleTemp <= 90f && endAngleTemp <= 150f && endAngleTemp >= 90f) {
        	...
        	showZoomValue = (showNum >=9.0f?showNum+1:showNum<=3?1:showNum);
